(function() {
  ...
  titleEl.textContent = 'LAST MODIFIED';
  ...
  if (lastMod && lastMod !== '0') {
    body.appendChild(row('Last Modified', lastMod));
    var parsed = new Date(lastMod);
    if (!isNaN(parsed)) {
      body.appendChild(row('ISO', parsed.toISOString()));
    }
  } else {
    body.appendChild(row('Last Modified', 'Not available'));
  }

  var note = document.createElement('div');
  note.style.cssText = 'padding:8px 0;color:#555;font-size:10px';
  note.textContent = 'This does not work reliably on dynamic pages (SPAs, social media, etc). Works best on static HTML pages.';
