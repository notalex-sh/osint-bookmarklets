(function() {
  ...
  var threadsUrl = username ? 'https://www.threads.net/@' + username : null;
  ...
  if (threadsUrl) {
    ...
    urlRow.textContent = threadsUrl;
    body.appendChild(urlRow);
  } else {
    var err = document.createElement('div');
    err.style.cssText = 'padding:8px 0;color:#777';
    err.textContent = 'Navigate to an Instagram profile first.';
  ...
  if (threadsUrl) {
    ...
    openBtn.textContent = 'Open Threads';
    ...
    openBtn.onclick = function() { window.open(threadsUrl, '_blank'); };
    actions.appendChild(openBtn);
  }

  win.appendChild(bar);
  win.appendChild(body);
  if (threadsUrl) win.appendChild(actions);
